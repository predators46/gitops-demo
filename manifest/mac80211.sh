...
mac80211_hostapd_setup_base() {
	...
	[ -n "$ieee80211n" ] && {
		...
		ht_cap_mask=0
		for cap in $(ifconfig phy "$phy" info | grep 'Capabilities:' | cut -d: -f2); do
...
mac80211_ifconfig_interface_add() {
	local phy="$1"
	local ifname="$2"
	local type="$3"
	local wdsflag="$4"
	...
	ifconfig phy "$phy" interface add "$ifname" type "$type" $wdsflag >/dev/null 2>&1
	rc="$?"
	...
	[ "$rc" = 233 ] && {
		...
		case "$(ifconfig dev $ifname info | grep "^\ttype" | cut -d' ' -f2- 2>/dev/null)" in
			"AP")
				[ "$type" = "__ap" ] && rc=0
				;;
			"IBSS")
	...
	[ "$rc" = 233 ] && {
		ifconfig dev "$ifname" del >/dev/null 2>&1
		[ "$?" = 0 ] && {
			sleep 1
...
mac80211_prepare_vif() {
	...
	case "$mode" in
		adhoc)
			mac80211_ifconfig_interface_add "$phy" "$ifname" adhoc || return
		;;
		ap)
		...
		;;
		mesh)
			mac80211_ifconfig_interface_add "$phy" "$ifname" mp || return
		;;
		monitor)
			mac80211_ifconfig_interface_add "$phy" "$ifname" monitor || return
		;;
		sta)
			local wdsflag=
			[ "$enable" = 0 ] || staidx="$(($staidx + 1))"
			[ "$wds" -gt 0 ] && wdsflag="4addr on"
			mac80211_ifconfig_interface_add "$phy" "$ifname" managed "$wdsflag" || return
			if [ "$wds" -gt 0 ]; then
				ifconfig "$ifname" set 4addr on
			else
				ifconfig "$ifname" set 4addr off
			fi
			[ "$powersave" -gt 0 ] && powersave="on" || powersave="off"
			ifconfig "$ifname" set power_save "$powersave"
		;;
	esac

	case "$mode" in
		monitor|mesh)
			[ "$auto_channel" -gt 0 ] || ifconfig dev "$ifname" set channel "$channel" $ifconfig_htmode
		;;
	esac
...
mac80211_prepare_ifconfig_htmode() {
	case "$htmode" in
		VHT20|HT20) ifconfig_htmode=HT20;;
		HT40*|VHT40|VHT160)
			case "$band" in
				2g)
					case "$htmode" in
						HT40+) ifconfig_htmode="HT40+";;
						HT40-) ifconfig_htmode="HT40-";;
						*)
							if [ "$channel" -lt 7 ]; then
								ifconfig_htmode="HT40+"
							else
								ifconfig_htmode="HT40-"
							fi
						;;
				...
				*)
					case "$(( ($channel / 4) % 2 ))" in
						1) ifconfig_htmode="HT40+" ;;
						0) ifconfig_htmode="HT40-";;
					esac
				;;
			esac
			[ "$auto_channel" -gt 0 ] && ifconfig_htmode="HT40+"
		;;
		VHT80)
			ifconfig_htmode="80MHZ"
		;;
		NONE|NOHT)
			ifconfig_htmode="NOHT"
		;;
		*) ifconfig_htmode="" ;;
	esac
}
...
mac80211_setup_adhoc() {
	...
	ifconfig dev "$ifname" set type ibss
	ifconfig dev "$ifname" ibss join "$ssid" $freq $ifconfig_htmode fixed-freq $bssid \
...
mac80211_setup_mesh() {
	...
	ifconfig dev "$ifname" mesh join "$ssid" freq $freq $ifconfig_htmode \
		${mcval:+mcast-rate $mcval} \
		beacon-interval $beacon_int
}
...
mac80211_setup_vif() {
	...
	if [ "$mode" != "ap" ] || [ "$ifname" = "$ap_ifname" ]; then
		...
		[ -z "$vif_txpower" ] || ifconfig dev "$ifname" set txpower fixed "${vif_txpower%%.*}00"
	fi
	...
	case "$mode" in
		mesh)
			...
			for var in $MP_CONFIG_INT $MP_CONFIG_BOOL $MP_CONFIG_STRING; do
				...
				[ -n "$mp_val" ] && ifconfig dev "$ifname" set mesh_param "$var" "$mp_val"
			done
		;;
		adhoc)
			wireless_vif_parse_encryption
...
drv_mac80211_setup() {
	...
	ifconfig phy "$phy" set antenna_gain $antenna_gain >/dev/null 2>&1
	...
	[ -n "${NEWAPLIST}" ] && mac80211_ifconfig_interface_add "$phy" "${NEWAPLIST%% *}" __ap
	local add_ap=0
		...
		if [ -n "$(ubus list | grep hostapd.$primary_ap)" ]; then
			...
			[ "${NEW_MD5}" = "${OLD_MD5}" ] || {
				...
				if [ "$no_reload" != "0" ]; then
					...
					mac80211_vap_cleanup none "$(uci -q -P /var/state get wireless._${phy}.umlist)"
					sleep 2
					mac80211_ifconfig_interface_add "$phy" "${NEWAPLIST%% *}" __ap
					for_each_interface "sta adhoc mesh monitor" mac80211_prepare_vif
				fi
			}
		fi
